...
struct ecryptfs_crypto_list
{
    struct list_head list;
    unsigned long len;
    unsigned long offset;
};
#define list_for_each_head_tail(pos, n, head, tail) \
         for(pos = (head), n = pos->next; pos != (tail)->next; \
            pos = n, n = pos->next)
extern int init_crypto_len_list(struct ecryptfs_crypto_list* _listhead);

extern int free_crypto_len_list(struct ecryptfs_crypto_list* _listhead);
extern int get_and_remove_crypto_idx_by_offset_and_len(struct ecryptfs_crypto_list* _listhead, unsigned long offset, unsigned long len, struct ecryptfs_crypto_list *ret_listhead, int* ret_start_offset, int* ret_end_offset);
extern int get_crypto_idx_by_offset_and_len(struct ecryptfs_crypto_list* _listhead, unsigned long offset, unsigned long len, struct ecryptfs_crypto_list **ret_listhead, struct ecryptfs_crypto_list **ret_listail,int* ret_start_offset, int* ret_end_offset);
extern void set_crypto_len_by_offset(struct ecryptfs_crypto_list* _listhead, unsigned long offset, unsigned long len);
...
#define ECRYPTFS_I_SIZE_INITIALIZED   0x00004000
	...
	struct ecryptfs_crypto_list crypto_len_listhead;
};
...
int ecryptfs_decrypt_buf(struct ecryptfs_crypt_stat *crypt_stat,
                             ...
                             unsigned char *iv);
/*For asynchronous IO*/
struct ecryptfs_async_req {
	struct inode *lower_inode;/*The inode of the lower filesystem*/
	void *data; /*Address of NVM space*/
	loff_t offset; 
	size_t size;
};
...
struct todo_list_item {
	...
	struct ecryptfs_async_req req;
	int result;/*async result, after writing*/
};
...
struct ecryptfs_async_ctx {
	struct locked_list todo_list;
	struct work_struct io_task;
	struct workqueue_struct *ecryptfs_async_wq;
};
extern struct ecryptfs_async_ctx *async_io_ctx;
